...
std::atomic<uint64_t> Offer::offer_id_ = 0;
std::atomic<uint64_t> Deal::deal_id_ = 0;
std::atomic<uint64_t> UserData::user_id_ = 0;

TEST_CASE("Register User", "[market]") {
    Market market;

    const auto user_id1 = market.RegisterUser("user1", 0);
    const auto user_id2 = market.RegisterUser("user2", 0);
...
TEST_CASE("Deposit currency", "[market]") {
    Market market;
    const auto user_id = market.RegisterUser("user1", 0);
...
TEST_CASE("Partial fullfillment of buy request with prioritization") {
    ...
    const auto user_id1 = market.RegisterUser("user1", 0);
    const auto user_id2 = market.RegisterUser("user2", 0);
    const auto user_id3 = market.RegisterUser("user3", 0);
    {
        auto offer_type = OfferType::SELL;
        int price = 61;
        size_t amount = 10;
    ...
    SECTION("Fullfill partial offer") {
        auto user_id4 = market.RegisterUser("user4", 0);
        {
            auto offer_type = OfferType::SELL;
            int price = 60;
            size_t amount = 10;
...
TEST_CASE("Get quotes") {
    ...
    SECTION("Ask quote") {
        Market market;
        auto user_id1 = market.RegisterUser("user1", 0);
    ...
    SECTION("All quotes and spread") {
        Market market;
        auto user_id1 = market.RegisterUser("user1", 0);
        auto user_id2 = market.RegisterUser("user2", 0);
...
TEST_CASE("Offer rejection") {
    SECTION("Post and decline one offer") {
        Market market;
        auto user_id = market.RegisterUser("user", 0);
        {
            OfferType offer_type = OfferType::BUY;
            int price = 80;
            size_t amount = 10;
